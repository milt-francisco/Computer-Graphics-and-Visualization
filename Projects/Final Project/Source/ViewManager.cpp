	...
	ShaderManager *pShaderManager)
{
	...
	// variables to cycle through orthographic views
	cycleView = 0;
	keyPressed = false;
}
...
GLFWwindow* ViewManager::CreateDisplayWindow(const char* windowTitle)
{
	...
	// this callback is used to receive scroll wheel movement events
 ...
 *  the scroll wheel is moved within the active GLFW display
 *  window.
 ***********************************************************/
...
void ViewManager::ProcessKeyboardEvents()
{
	...
	// change between different orthographic projection views
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_PRESS && !keyPressed) 
	{
		// Prevents rapid cycling of views while key is held down
		keyPressed = true;

		// front orthographic view
		if (cycleView == 0) {

			// change to multi-view orthographic projection
			bOrthographicProjection = true;

			// change camera settings to front view
		...
		// side orthographic view
			...
			// change the camera settings to side view
			g_pCamera->Position = glm::vec3(15.0f, 4.0f, 3.0f);
			g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
		...
		// top orthographic view
			...
			// change the camera settings to top view (looking to front)
			g_pCamera->Position = glm::vec3(0.0f, 10.0f, 3.0f);
	...
	// waits until key is released before allowing next cycle
	if (glfwGetKey(m_pWindow, GLFW_KEY_O) == GLFW_RELEASE) {

		// allows entrance into above conditional
		keyPressed = false;
	}
	...
	if (glfwGetKey(m_pWindow, GLFW_KEY_P) == GLFW_PRESS) {
		...
		// change camera settings to show a perspective view
		g_pCamera->Position = glm::vec3(0.0f, 5.5f, 13.5f);
		g_pCamera->Front = glm::vec3(0.0f, -0.5f, -2.0f);
		g_pCamera->Up = glm::vec3(0.0f, 1.0f, 0.0f);
		g_pCamera->Zoom = 80;
	}
